...
namespace aasm::details {
    constexpr std::optional<std::uint8_t> prefix(const std::uint8_t size, const GPReg src, const Address& dest) noexcept {
    ...
    constexpr std::optional<std::uint8_t> prefix(const std::uint8_t to_size, const std::uint8_t from_size, const GPReg src, const Address& dest) noexcept {
        auto code = R(src) | X(dest);
        if (const auto base = dest.base(); base.has_value()) {
            code |= B(base.value());
        }
    ...
    constexpr std::optional<std::uint8_t> prefix(const std::uint8_t to_size, const std::uint8_t from_size, const GPReg src, const GPReg dest) noexcept {
        auto code = R(src) | B(dest);
        if (to_size == 8) {
            code |= constants::REX_W;
        } else {
    ...
    template<typename Op2, CodeBuffer Buffer>
    requires std::is_same_v<Op2, Address> || std::is_same_v<Op2, GPReg>
    constexpr void emit_op_prologue(Buffer& buffer, const std::uint8_t to_size, const std::uint8_t from_size, const GPReg op1, const Op2& op2) {
        if (to_size == 2) {
            add_word_op_size(buffer);
        }

        if (const auto prefix = details::prefix(to_size, from_size, op1, op2); prefix.has_value()) {
            buffer.emit8(prefix.value());
        }
    }
    ...
    public:
        ...
        constexpr void encode_MR(const std::uint8_t to_size, const std::uint8_t from_size, const GPReg src, const GPReg dest) {
            emit_op_prologue(m_buffer, to_size, from_size, src, dest);
