...
namespace aasm::details {
    ...
    public:
        ...
        requires std::is_same_v<Op, Address> || std::is_same_v<Op, XmmReg> || std::is_same_v<Op, GPReg>
        [[nodiscard]]
        constexpr std::optional<Relocation> encode_A(const std::uint8_t size, const Op& src, const Reg dest) {
            emit_opcodes(m_prefixes);
            if (const auto prefix = EncodeUtils::prefix(size, dest, src); prefix.has_value()) {
                m_buffer.emit8(prefix.value());
            }
