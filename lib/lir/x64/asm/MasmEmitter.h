...
public:
    ...
    template<XVRegVariant Op>
    constexpr void cvtfp2si(const std::uint8_t from_size, const std::uint8_t to_size, const Op src, const aasm::GPReg dst) {
        switch (from_size) {
            case cst::DWORD_SIZE: m_asm.cvtss2si(to_size, src, dst); break;
            case cst::QWORD_SIZE: m_asm.cvtsd2si(to_size, src, dst); break;
            default: std::unreachable();
        }
    }

    template<GPVRegVariant Op>
    constexpr void cvtsi2fp(const std::uint8_t from_size, const std::uint8_t to_size, const Op& src, const aasm::XmmReg dst) {
        switch (to_size) {
            case cst::DWORD_SIZE: m_asm.cvtsi2ss(from_size, src, dst); break;
            case cst::QWORD_SIZE: m_asm.cvtsi2sd(from_size, src, dst); break;
            default: std::unreachable();
        }
    }
