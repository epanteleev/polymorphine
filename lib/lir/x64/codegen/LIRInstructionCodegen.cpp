...
GPOp LIRInstructionCodegen::convert_to_gp_op(const LIROperand &val) const {
...
aasm::GPReg LIRInstructionCodegen::convert_to_gp_reg(const LIRVal &val) const {
...
void LIRInstructionCodegen::add_i(const LIRVal &out, const LIROperand &in1, const LIROperand &in2) {
...
void LIRInstructionCodegen::sub_i(const LIRVal &out, const LIROperand &in1, const LIROperand &in2) {
    const auto out_reg = m_reg_allocation[out];
    const auto in1_reg = convert_to_gp_op(in1);
    const auto in2_reg = convert_to_gp_op(in2);
...
void LIRInstructionCodegen::setcc_i(const LIRVal &out, const aasm::CondType cond_type) {
    const auto out_reg = m_reg_allocation[out];
...
void LIRInstructionCodegen::cmov_i(aasm::CondType cond_type, const LIRVal& out, const LIROperand& in1, const LIROperand& in2) {
    const auto out_reg = m_reg_allocation[out];
    const auto in1_reg = convert_to_gp_op(in1);
    const auto in2_reg = convert_to_gp_op(in2);
    CMovGPEmit emitter( m_clobber_regs, m_as, cond_type, out.size());
    emitter.emit(out_reg, in1_reg, in2_reg);
}

void LIRInstructionCodegen::cmp_i(const LIROperand &in1, const LIROperand &in2) {
    const auto in1_reg = convert_to_gp_op(in1);
    const auto in2_reg = convert_to_gp_op(in2);
    CmpGPEmit::emit(m_as, in1.size(), in1_reg, in2_reg);
}

void LIRInstructionCodegen::mov_i(const LIRVal &in1, const LIROperand &in2) {
...
void LIRInstructionCodegen::store_i(const LIRVal &pointer, const LIROperand &value) {
...
void LIRInstructionCodegen::up_stack(const aasm::GPRegSet& reg_set, const std::size_t caller_overflow_area_size) {
...
void LIRInstructionCodegen::down_stack(const aasm::GPRegSet& reg_set, const std::size_t caller_overflow_area_size) {
    auto size_to_adjust = caller_overflow_area_size;
    if (const auto remains = (m_reg_allocation.frame_size()+caller_overflow_area_size+reg_set.size()*8) % call_conv::STACK_ALIGNMENT; remains != 0L) {
        size_to_adjust += remains;
    }
...
void LIRInstructionCodegen::prologue(const aasm::GPRegSet &reg_set, const std::size_t caller_overflow_area_size) {
...
void LIRInstructionCodegen::epilogue(const aasm::GPRegSet &reg_set, const std::size_t caller_overflow_area_size) {
    if (is_no_prologue(m_reg_allocation) && caller_overflow_area_size == 0) {
        return;
    }
...
void LIRInstructionCodegen::copy_i(const LIRVal &out, const LIROperand &in) {
    const auto out_reg = m_reg_allocation[out];
    CopyGPEmit::emit(m_as, out.size(), out_reg, convert_to_gp_op(in));
}

void LIRInstructionCodegen::load_i(const LIRVal &out, const LIRVal &pointer) {
    const auto out_reg = m_reg_allocation[out];
    const auto pointer_reg = m_reg_allocation[pointer];
    LoadGPEmit::emit(m_as, out.size(), out_reg, pointer_reg);
}

void LIRInstructionCodegen::jmp(const LIRBlock *bb) {
    const auto target = m_bb_labels.at(bb);
    m_as.jmp(target);
}

void LIRInstructionCodegen::jcc(const aasm::CondType cond_type, const LIRBlock *on_true, const LIRBlock *on_false) {
    const auto target_false = m_bb_labels.at(on_false);
    m_as.jcc(aasm::invert(cond_type), target_false);
}

void LIRInstructionCodegen::call(const LIRVal &out, const std::string_view name, std::span<LIRVal const> args, const LIRLinkage linkage) {
    const auto [symbol, _] = m_symbol_tab.add(name, cvt_linkage(linkage));
    m_as.call(symbol);
}

void LIRInstructionCodegen::ret(const std::span<LIRVal const> ret_values) {
    m_as.ret();
}