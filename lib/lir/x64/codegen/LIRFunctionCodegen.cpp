...
namespace details {
    class LIRInstructionCodegen final: public LIRVisitor, public LIROperandMapping {
    public:
        explicit LIRInstructionCodegen(MasmEmitter& as, TemporalRegs regs, aasm::SymbolTable& symbol_table, const LIRBlock* next, std::unordered_map<const LIRBlock*, aasm::Label>& bb_labels) noexcept:
...
void LIRFunctionCodegen::traverse_instructions() {
    ...
    for (const auto [idx, bb]: std::ranges::views::enumerate(m_preorder)) {
        ...
        for (auto& inst: bb->instructions()) {
            details::LIRInstructionCodegen codegen(m_as, inst.temporal_regs(), m_sym_tab, m_next, m_bb_labels);
            inst.visit(codegen);
        }
    }
}