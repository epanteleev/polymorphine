...
static Module ret_i32(const std::int32_t value) {
    ...
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i32(), {}, "ret_one", FunctionVisibility::DEFAULT);
...
static Module ret_i64(const std::int64_t value) {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i64(), {}, "ret_one", FunctionVisibility::DEFAULT);
...
static Module ret_i8_u8(const std::int8_t value) {
    ModuleBuilder builder;
    {
        const auto prototype = builder.add_function_prototype(SignedIntegerType::i8(), {}, "ret_i8", FunctionVisibility::DEFAULT);
        const auto fn_builder = builder.make_function_builder(prototype);
        const auto data = fn_builder.value();
        data.ret(Value::i8(value));
    }
    {
        const auto prototype = builder.add_function_prototype(UnsignedIntegerType::u8(), {}, "ret_u8", FunctionVisibility::DEFAULT);
        const auto fn_builder = builder.make_function_builder(prototype);
        const auto data = fn_builder.value();
...
static Module ret_i32_arg() {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i32(), {SignedIntegerType::i32()}, "ret_i32", FunctionVisibility::DEFAULT);
    const auto fn_builder = builder.make_function_builder(prototype);
...
static Module add_i32_args(const NonTrivialType* ty) {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(ty, {ty, ty}, "add", FunctionVisibility::DEFAULT);
    const auto fn_builder = builder.make_function_builder(prototype);
    const auto data = fn_builder.value();
    const auto arg0 = data.arg(0);
...
static Module branch() {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i32(), {}, "ret", FunctionVisibility::DEFAULT);
    const auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
...
static Module is_predicate_base(Fn&& fn, const IntegerType* ty, const Value& threshold) {
    ModuleBuilder builder;
    {
        const auto prototype = builder.add_function_prototype(ty, {ty}, "is_neg", FunctionVisibility::DEFAULT);
        auto fn_builder = builder.make_function_builder(prototype);
        fn(fn_builder.value(), IcmpPredicate::Lt,  threshold);
    }
    {
        const auto prototype = builder.add_function_prototype(ty, {ty}, "is_le", FunctionVisibility::DEFAULT);
        auto fn_builder = builder.make_function_builder(prototype);
        fn(fn_builder.value(), IcmpPredicate::Le, threshold);
    }
    {
        const auto prototype = builder.add_function_prototype(ty, {ty}, "is_gt", FunctionVisibility::DEFAULT);
        auto fn_builder = builder.make_function_builder(prototype);
        fn(fn_builder.value(), IcmpPredicate::Gt, threshold);
    }
    {
        const auto prototype = builder.add_function_prototype(ty, {ty}, "is_eq", FunctionVisibility::DEFAULT);
        auto fn_builder = builder.make_function_builder(prototype);
        fn(fn_builder.value(), IcmpPredicate::Eq, threshold);
    }
    {
        const auto prototype = builder.add_function_prototype(ty, {ty}, "is_ne", FunctionVisibility::DEFAULT);
        auto fn_builder = builder.make_function_builder(prototype);
        fn(fn_builder.value(), IcmpPredicate::Ne, threshold);
    }
    {
        const auto prototype = builder.add_function_prototype(ty, {ty}, "is_ge", FunctionVisibility::DEFAULT);
        auto fn_builder = builder.make_function_builder(prototype);
...
static Module stack_alloc(const Value& val) {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(val.type(), {}, "stackalloc", FunctionVisibility::DEFAULT);
    const auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
...
static Module swap(const PrimitiveType* ty) {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(VoidType::type(), {PointerType::ptr(), PointerType::ptr()}, "swap", FunctionVisibility::DEFAULT);
    auto fn_builder = builder.make_function_builder(prototype);
    const auto data = fn_builder.value();
    const auto arg0 = data.arg(0);
    const auto arg1 = data.arg(1);
