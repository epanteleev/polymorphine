...
static Module sext_cvt(const SignedIntegerType* from, const SignedIntegerType* to) {
    ...
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i32(), {from}, "cvt", FunctionVisibility::DEFAULT);
...
static Module trunc_cvt(const IntegerType* from, const IntegerType* to) {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(from, {from}, "cvt", FunctionVisibility::DEFAULT);
