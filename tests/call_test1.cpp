...
static Module call_external() {
    ...
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i32(), {}, "call_external", FunctionVisibility::DEFAULT);
    auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
    const auto cont = data.create_basic_block();
    const auto proto = builder.add_function_prototype(SignedIntegerType::i32(), {}, "return_constant", FunctionVisibility::EXTERN);
...
static Module call_external_with_args() {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i32(), {}, "call_external_with_args", FunctionVisibility::DEFAULT);
    const auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
    const auto cont = data.create_basic_block();
    const auto proto = builder.add_function_prototype(SignedIntegerType::i32(), {SignedIntegerType::i32(), SignedIntegerType::i32()}, "add", FunctionVisibility::EXTERN);
...
static Module clamp() {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i64(), {SignedIntegerType::i64(), SignedIntegerType::i64(), SignedIntegerType::i64()}, "clamp", FunctionVisibility::DEFAULT);
    const auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
    ...
    const auto cont = data.create_basic_block();
    const auto proto_min = builder.add_function_prototype(SignedIntegerType::i64(), {SignedIntegerType::i64(), SignedIntegerType::i64()}, "min", FunctionVisibility::EXTERN);
    const auto clamped = data.call(proto_min, cont, {a, max_val});
    data.switch_block(cont);

    const auto cont1 = data.create_basic_block();
    const auto proto_max = builder.add_function_prototype(SignedIntegerType::i64(), {SignedIntegerType::i64(), SignedIntegerType::i64()}, "max", FunctionVisibility::EXTERN);
...
static Module argument_shuffle() {
    ...
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i64(), {SignedIntegerType::i32(), SignedIntegerType::i32(), SignedIntegerType::i32(),
                                                          SignedIntegerType::i32(), SignedIntegerType::i32(), SignedIntegerType::i32()}, "arg_shuffle", FunctionVisibility::DEFAULT);
    auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
    ...
    const auto arg_locator = builder.add_function_prototype(SignedIntegerType::i64(), {SignedIntegerType::i32(), SignedIntegerType::i32(),
                                                                               ...
                                                                               SignedIntegerType::i32(), SignedIntegerType::i32()},
                                                     "arg_locator", FunctionVisibility::EXTERN);
...
static Module argument_shuffle7(const IntegerType* ty) {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i64(), {ty, ty, ty, ty, ty, ty, ty}, "arg_shuffle", FunctionVisibility::DEFAULT);
    const auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
    ...
    const auto arg_locator = builder.add_function_prototype(SignedIntegerType::i64(), {ty, ty, ty, ty, ty, ty, ty}, "arg_locator7", FunctionVisibility::EXTERN);
    const auto cont = data.create_basic_block();
...
static Module argument_shuffle8(const IntegerType* ty) {
    ModuleBuilder builder;
    const auto prototype = builder.add_function_prototype(SignedIntegerType::i64(), {ty, ty, ty, ty, ty, ty, ty, ty}, "arg_shuffle", FunctionVisibility::DEFAULT);
    auto fn_builder = builder.make_function_builder(prototype);
    auto data = fn_builder.value();
    ...
    const auto arg_locator = builder.add_function_prototype(SignedIntegerType::i64(), {ty, ty, ty, ty, ty, ty, ty, ty, ty}, "arg_locator8", FunctionVisibility::EXTERN);
    const auto cont = data.create_basic_block();
